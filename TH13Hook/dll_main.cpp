...
bool HookStart(void) {
	if(th13_global->d3d9_ptr == NULL) {
		...
		if(!WriteCode(reinterpret_cast<unsigned char *>(&p_Direct3DCreate9), 0x4A2298, reinterpret_cast<const unsigned char *>(&new_code), sizeof(new_code))) {
			return false;
		}
		return true;
	}
...
bool CheckTH13(void) {
	...
	const unsigned int file_size = 790528;
	const unsigned int checksum = 0x1D7C0BB4;
