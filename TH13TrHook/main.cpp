...
TH13Global * const th13_global = reinterpret_cast<TH13Global *>(0x004B62A8);
...
void *(__cdecl * const TH13Malloc)(unsigned int) = reinterpret_cast<void *(__cdecl *)(unsigned int)>(0x004754DD);
...
#else
	if(th13_global->engine_mode != 1) {
		return;
	}
#endif
	if(::GetKeyState(VK_F1) < 0) {
		th13_global->flags |= 0x80;
	}
	if(::GetKeyState(VK_F2) < 0) {
		th13_global->flags &= 0xFFFFFF7F;
	}
...
void _main() {
	{
		...
		bool result;
		// 霊夢ボムのSEを変更
		result = ChangeCode(0x0040A556, &old_code, &new_code, 1);
		BOOST_ASSERT(result);
		result = ChangeCode(0x0040A60F, &old_code, &new_code, 1);
		BOOST_ASSERT(result);
		result = ChangeCode(0x0040AB65, &old_code, &new_code, 1);
		BOOST_ASSERT(result);
		// 妖夢ボムのSEを変更
		result = ChangeCode(0x0040C295, &old_code, &new_code, 1);
		BOOST_ASSERT(result);
	}
	{
		...
		// 早苗ボムのSEを変更
		const bool result = ChangeCode(0x0040B7AC, &old_code, &new_code, 1);
		BOOST_ASSERT(result);
	}
}
